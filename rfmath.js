//1 / (2*PI*sqrt(C*L))
// Resonant frequency (f) is the relationship between
// inductance (L) and capcitance (C).
function frequency (L, C) {
    L = Number(L);
    C = Number(C);
    if (isNaN(L)  || isNaN (C)) return null;
    if (L <= 0 || C <= 0) return null;
    L /= 1000; // Assume L is in nanoHenries. Convert to micro.
    C /= 1000; // Assume C is in nanoFarads. Convert to micro.
...
// return a named array of L and C with the best values to hit the
// desired frequency and Q.
